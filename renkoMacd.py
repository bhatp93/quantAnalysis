...
'''
db = msql.connect(user='stockdbAdmin',passwd='S3cret!',host='mysqldb',database='stockdb')
cur = db.cursor()
cur.execute("select * from stock_listings;")
f=cur.fetchall()
tickers = []
for stock in f:
    tickers.append(stock[3])  


tickers = ['SMCI','BAC','GOOGL']
'''

def calculate(tickers: list[str] ):
    ohlc_intraday = {} # directory with ohlc value for each stock            
    #key_path = "D:\\Udemy\\Quantitative Investing Using Python\\1_Getting Data\\AlphaVantage\\key.txt"
    ts = TimeSeries(key='IGCQLKLSU8X29E0E', output_format='pandas')
